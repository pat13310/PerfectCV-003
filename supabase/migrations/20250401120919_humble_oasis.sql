/*
  ...
  1. Changes
    - Drop all existing policies and triggers to avoid conflicts
...
DROP POLICY IF EXISTS "Users can insert experiences for their CVs" ON experiences;
DROP POLICY IF EXISTS "Users can modify their own experiences" ON experiences;
DROP POLICY IF EXISTS "Users can remove their own experiences" ON experiences;
...
CREATE POLICY "allow_select_own_experiences"
  ON experiences
  FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY "allow_insert_own_experiences"
  ON experiences
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
CREATE POLICY "allow_update_own_experiences"
  ON experiences
  FOR UPDATE
  TO authenticated
  USING (
...
CREATE POLICY "allow_delete_own_experiences"
  ON experiences
  FOR DELETE
  TO authenticated
  USING (
